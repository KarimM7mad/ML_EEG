...
elif bool(re.match("([a-zA-Z 0-9_])+(.csv)$", file1)) == False or \
        bool(re.match("([a-zA-Z 0-9_])+(.csv)$", file2)) == False or \
        bool(re.match("([a-zA-Z 0-9_])+(.csv)$", file3)) == False:
    print("Can't execute , regex doesn't match")
else:
    concatFiles(file1Path="dataset/" + file1,file2Path="dataset/" + file2,desFilePath="dataset/" + file3)
#    if sys.argv[4] == "1":
